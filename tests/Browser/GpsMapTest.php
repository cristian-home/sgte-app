...
test('admin sees the /gps/map page with the services panel + symbol legend', function (): void {
    ...
    $this->browse(function (Browser $browser) use ($user): void {
        $browser->loginAs($user)
            ...
            // The services panel lists each active service alongside the
            // colour-coded symbol legend.
            ->waitForText('Servicios', 10)
            ->assertSee('GPS99')
            ->assertSee('Juan Gómez')
            ->waitForText('Símbolos', 10)
            ->assertSee('Vehículo (GPS)')
            ->assertSee('Ruta confirmada')
            ->assertSee('Ruta estimada')
            ->assertDontSee('auto-generated by Blueprint')
            ->screenshot('gps-map-admin');
    });
});

test('clicking a service in the panel marks it as selected', function (): void {
    $user = gpsAuthenticateAsSuperAdmin();
    $service = gpsSeedActiveServiceWithLocation();

    $this->browse(function (Browser $browser) use ($user, $service): void {
        $row = '[data-dusk="service-item-'.$service->id.'"]';

        $browser->loginAs($user)
            ->visit('/gps/map')
            ->waitForText('Servicios', 10)
            ->waitFor($row, 10)
            ->assertSeeIn($row, 'GPS99')
            ->assertAttribute($row, 'aria-pressed', 'false')
            ->click($row)
            ->waitUsing(5, 100, fn () => $browser->attribute($row, 'aria-pressed') === 'true')
            ->assertAttribute($row, 'aria-pressed', 'true')
            ->screenshot('gps-map-service-selected');
    });
});
