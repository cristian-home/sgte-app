...
- [ ] **AC1**: WHEN an admin or operator navigates to `/drivers` THEN the page renders a paginated `<DataTable>` (not a JSON dump) with columns: **Documento**, **Nombre**, **Categoría**, **Licencia** (status pill), **Seg. Social** (badge), **Vinculación** (badge), **Acciones**.
- [ ] **AC2**: WHEN a driver's `license_due_date` is null OR strictly before today THEN the row background is `bg-destructive/10` AND the **Licencia** pill renders with the `destructive` Badge variant and `!` suffix.
- [ ] **AC3**: WHEN a driver's `license_due_date` is `>= today` AND `<= today + 30 days` AND has not expired THEN the row background is `bg-amber-100/60` (light) / `bg-amber-900/20` (dark) AND the **Licencia** pill renders with the `secondary` Badge variant.
- [ ] **AC4**: WHEN a driver's `license_due_date` is `> today + 30 days` THEN the row background is unchanged AND the **Licencia** pill renders with the `outline` Badge variant.
- [ ] **AC5**: WHEN the user applies the **Estado** filter THEN only rows whose `drivers.active` matches the selected boolean remain visible.
- [ ] **AC6**: WHEN the user applies the **Categoría** filter (C1 / C2 / C3) THEN only rows whose `drivers.license_category` matches the selected value remain visible.
- [ ] **AC7**: WHEN the user picks a municipality from the `<MunicipalityCombobox />` rendered above the table THEN only rows whose `drivers.municipality_id` matches remain.
- [ ] **AC8**: WHEN the user applies the **Documentos** (license_status) filter with value `expired` THEN only rows whose license is expired or null are visible; WHEN `expiring_soon` THEN only rows in the 30-day window (and not expired); WHEN `ok` THEN only rows with license `> today + 30 days` are visible.
- [ ] **AC9**: WHEN the user applies the **Seguridad Social** filter (Sí / No) THEN only rows whose `drivers.has_social_security` matches remain visible.
- [ ] **AC10**: WHEN the user clicks the **Crear Conductor** action on the index THEN a new `<DriverCreateDialog />` modal opens. The modal contains the new shared `<DriverForm />` component organized into five `<h3>` sections in this order: **Identificación**, **Datos de Contacto**, **Licencia**, **Afiliaciones**, **Estado**. On successful submit the modal closes AND the index refreshes with the new driver visible.
- [ ] **AC11**: WHEN the user navigates to `/drivers/create` directly THEN the standalone create page renders the same `<DriverForm />` (no modal wrapper) with a Guardar / Cancelar action bar. Cancelar returns to `/drivers`.
- [ ] **AC12**: WHEN the user clicks the **Documento** link in any row THEN the app navigates to `/drivers/{id}` AND the show page renders five Card sections in this order: **Header card** (nombre completo + identificación + estado activo badge + Cuenta Vinculada subsection + edit button), **Información Personal**, **Licencia y Seguridad Social**, **Afiliaciones**, **Servicios Recientes** (last 5 services).
- [ ] **AC13**: WHEN the driver has `user_id` set THEN the header card's **Cuenta Vinculada** subsection displays the linked user's email; WHEN `user_id` is null THEN it displays "Sin vínculo".
- [ ] **AC14**: WHEN the **Servicios Recientes** card has zero services THEN it renders the empty state "Sin servicios registrados.".
- [ ] **AC15**: WHEN a user clicks any row in the dashboard's **Alertas de Documentos** panel that corresponds to a driver alert (`kind === 'driver'`) AND `days_remaining < 0` THEN the app navigates to `/drivers?filter[license_status]=expired`; WHEN `days_remaining >= 0` THEN it navigates to `/drivers?filter[license_status]=expiring_soon`. The previous bare `/drivers` placeholder link added in commit `ee453d7` is fully removed.
- [ ] **AC16**: WHEN a driver navigates to `/drivers` or `/drivers/{id}` THEN they receive a 403 (drivers do NOT hold `VIEW_DRIVERS` — they only see their own services through `/driver`).
- [ ] **AC17**: WHEN an accounting user navigates to `/drivers` or `/drivers/{id}` THEN they receive a 403.
- [ ] **AC18**: WHEN an unauthenticated user navigates to `/drivers` or `/drivers/{id}` THEN they are redirected to `/login`.
...
- [ ] **Task B1**: Paginate `DriverController@index` and enrich its payload.
...
- [ ] **Task B2**: Add the `license_status` filter callback.
...
- [ ] **Task B3**: Add `AllowedFilter::exact('has_social_security')` to the index `allowedFilters` array.

- [ ] **Task B4**: Expand `DriverController@show` to load relationships and recent services.
...
- [ ] **Task B5**: Update `DashboardController::buildDocumentAlerts` driver branch.
...
- [ ] **Task F1**: Create `resources/js/components/drivers/driver-license-pill.tsx`.
...
- [ ] **Task F2**: Create `resources/js/components/drivers/driver-form.tsx`.
...
- [ ] **Task F3**: Create `resources/js/components/drivers/driver-create-dialog.tsx`.
...
- [ ] **Task F4**: Create `resources/js/pages/drivers/columns.tsx`.
...
- [ ] **Task F5**: Rewrite `resources/js/pages/drivers/index.tsx`.
...
- [ ] **Task F6**: Rewrite `resources/js/pages/drivers/show.tsx`.
...
- [ ] **Task F7**: Rewrite `resources/js/pages/drivers/create.tsx` and `resources/js/pages/drivers/edit.tsx` to be real form pages.
...
- [ ] **Task T1 (Pest, backend)**: Add to `tests/Feature/Http/Controllers/DriverControllerTest.php`:
...
- [ ] **Task T2 (Pest, backend)**: Add filter tests to `DriverControllerTest`:
...
- [ ] **Task T3 (Pest, backend)**: Add show tests to `DriverControllerTest`:
...
- [ ] **Task T4 (Pest, backend)**: Add a dashboard test to `tests/Feature/DashboardTest.php`:
...
- [ ] **Task T5 (Pest, backend)**: Update existing `DriverControllerTest` if any test assumes the index response shape was a flat array — `test('index behaves as expected')` may need a paginated-shape adjustment.

- [ ] **Task T6 (Dusk, UI regression)**: Create `tests/Browser/DriversIndexAndShowTest.php` with four scenarios (consolidated single file, mirroring `VehiclesIndexAndShowTest.php`):
...
- [ ] Scenario 1: Admin sees the rebuilt index, applies license_status filter, sees tinted rows.
- [ ] Scenario 2: Admin clicks dashboard alert, lands on filtered drivers index.
- [ ] Scenario 3: Admin opens the show page and verifies all five cards + Cuenta Vinculada.
- [ ] Scenario 4: Operator can create + delete via the modal and the row-actions menu.
- [ ] Scenario 5: Driver and Accounting users receive 403 on `/drivers`.
